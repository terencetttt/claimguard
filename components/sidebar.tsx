...
export function Sidebar() {
  ...
  return <aside className="fixed inset-y-0 left-0 z-30 hidden w-[260px] flex-col border-r border-white/[.035] bg-[#08111F] px-4 py-6 text-[#9EABBE] shadow-[10px_0_35px_rgba(8,17,31,.04)] lg:flex">
    <div className="px-3"><Logo dark/></div>
    <nav className="mt-8 space-y-1">{nav.map(({label,href,icon:Icon})=>{const active=path===href||(href!=="/claims/new"&&path.startsWith(`${href}/`)&&!nav.some(item=>item.href!==href&&item.href.startsWith(`${href}/`)&&path.startsWith(item.href)));return <Link key={href} href={href} aria-current={active?"page":undefined} className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-medium transition ${active?"bg-[#14243A] text-white":"hover:bg-white/[.04] hover:text-white"}`}><Icon size={17}/><span>{label}</span>{active&&<span className="ml-auto size-1.5 rounded-full bg-[#4D8DFF]"/>}</Link>})}</nav>
