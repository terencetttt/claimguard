...
import { EVIDENCE_ACCEPT, EVIDENCE_LIMITS_HELP, validateEvidenceFile, validateLocalEvidenceAddition } from "@/lib/evidence-limits";
...
function ClaimIntakeSession(){
  ...
  if(!draft)return <div className="panel p-8 text-sm text-[#718094]">Loading claim draft…</div>;

  return <div className="claim-intake page-reveal grid min-w-0 items-start gap-5 xl:grid-cols-[200px_minmax(0,1fr)] 2xl:grid-cols-[216px_minmax(0,1fr)]"><aside className="panel h-fit p-4 xl:sticky xl:top-5 2xl:p-5"><div className="mb-4 border-b border-[#E4EAF1] pb-4"><p className="eyebrow">Claim progress</p><p className="mt-2 text-sm leading-5 text-[#718094]">Complete five evidence-led intake stages.</p></div><ClaimStepper current={step} onSelect={(value)=>{if(value<=step)setStep(value as ClaimStep)}}/><div className="mt-5 rounded-[14px] border border-[#DCE6F1] bg-[#F3F7FC] p-4 text-[12px] leading-5 text-[#62748B]"><div className="mb-2 flex items-center justify-between font-semibold text-[#34516F]"><span className="flex items-center gap-2"><Save size={14}/>Draft protection</span><span className={saveState==="Saving"?"text-[#946513]":"text-[#087B57]"}>{saveState}</span></div>Your progress is saved as a draft for this wallet.</div></aside><div className="min-w-0 space-y-4"><section className="panel overflow-hidden"><StepHeader step={step}/>{step===0&&<IncidentStep draft={draft} update={update} errors={errors}/>} {step===1&&<PolicyStep draft={draft} update={update} errors={errors}/>} {step===2&&<FinancialStep draft={draft} update={update} errors={errors}/>} {step===3&&<EvidenceStep draft={draft} update={update}/>} {step===4&&<ReviewStep draft={draft} update={update} walletAddress={wallet.address} onEdit={value=>setStep(value)} error={errors.declaration}/>}</section>{step<4&&<FollowingStages current={step}/>}{submitError&&<div className="rounded-xl border border-[#F1C5C9] bg-[#FFF5F5] px-4 py-3 text-xs font-medium leading-5 text-[#A14450]">{submitError}</div>}<div className="flex flex-wrap items-center justify-between gap-3 border-t border-[#DDE4ED] pt-4"><button type="button" onClick={saveNow} className="btn-secondary !min-h-[44px]"><Save size={16}/>Save draft</button><div className="flex flex-wrap gap-2"><button type="button" onClick={goBack} className="btn-secondary !min-h-[44px]" disabled={step===0}><ArrowLeft size={16}/>Back</button>{step<4?<button type="button" onClick={goNext} className="btn-primary !min-h-[44px] !px-5">Continue to {stepNames[step+1].replace(" information","").replace(" and submit","")} <ArrowRight size={16}/></button>:<button type="button" onClick={()=>void submit()} disabled={submitState==="submitting"} className="btn-primary !min-h-[44px] !px-5 disabled:cursor-not-allowed disabled:opacity-60">{submitState==="submitting"?"Submitting to Bradbury...":"Submit claim"} <ArrowRight size={16}/></button>}</div></div></div></div>;
}

function StepHeader({step}:{step:ClaimStep}){
  return <div className="border-b border-[#DDE4ED] bg-[linear-gradient(135deg,#fff,#F8FAFD)] px-5 py-5 sm:px-6">
    <div className="flex items-center gap-2 text-[#3975CC]"><ShieldCheck size={14}/><p className="eyebrow !text-[#3975CC]">Step {String(step+1).padStart(2,"0")} of 05</p></div>
    <h2 className="mt-2 text-[24px] font-semibold tracking-[-.035em] text-[#132033]">{stepNames[step]}</h2>
    <p className="mt-1 max-w-2xl text-[13px] leading-5 text-[#62748B]">{["Create a precise first account of the loss event.","Confirm the policy and coverage applying to this loss.","Document each component of the claimed financial loss.","Add and classify the records supporting this claim.","Check the complete claim record before local submission."][step]}</p>
  </div>
}
...
function IncidentStep({draft,update,errors}:StepProps){const x=draft.incident;const set=(value:Partial<typeof x>)=>update("incident",{...x,...value});return <div className="grid gap-x-5 gap-y-4 p-5 md:grid-cols-2 sm:p-6"><Field label="Incident type" error={errors.incidentType}><select className="field" value={x.incidentType} onChange={e=>set({incidentType:e.target.value})}><option>Collision</option><option>Theft</option><option>Fire damage</option><option>Flood damage</option><option>Vandalism</option></select></Field><Field label="Incident date" error={errors.incidentDate}><div className="relative"><input className="field" type="date" value={x.incidentDate} onChange={e=>set({incidentDate:e.target.value})}/><CalendarDays className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-[#7B899B]" size={17}/></div></Field><Field label="Location" error={errors.location}><div className="relative"><MapPin className="absolute left-4 top-1/2 -translate-y-1/2 text-[#7B899B]" size={17}/><input className="field !pl-11" value={x.location} onChange={e=>set({location:e.target.value})}/></div></Field><Field label="Vehicle registration" error={errors.vehicleRegistration}><input className="field uppercase" value={x.vehicleRegistration} onChange={e=>set({vehicleRegistration:e.target.value.toUpperCase()})}/></Field><div className="md:col-span-2"><Field label="Incident description" error={errors.description}><textarea className="field min-h-[120px] resize-y !py-4 leading-6" maxLength={1500} value={x.description} onChange={e=>set({description:e.target.value})}/><div className="mt-2 flex justify-between text-[11px] text-[#65758A]"><span>Include the sequence of events, parties involved, and visible damage.</span><span className="tabular-nums">{x.description.length} / 1,500</span></div></Field></div><fieldset className="md:col-span-2"><legend className="label">Was another party involved?</legend><div className="flex flex-wrap gap-3">{["Yes","No","Not sure"].map(value=><label key={value} className={`flex min-h-[48px] cursor-pointer items-center gap-2.5 rounded-xl border px-5 py-3 text-sm font-medium transition hover:border-[#9DBCF0] ${x.thirdPartyInvolved===value?"border-[#8DB4FF] bg-[#F1F6FF] text-[#245FAF]":"border-[#D6DFEA] bg-white text-[#526177]"}`}><input type="radio" name="party" checked={x.thirdPartyInvolved===value} onChange={()=>set({thirdPartyInvolved:value as ClaimDraft["incident"]["thirdPartyInvolved"]})} className="size-4 accent-[#4D8DFF]"/>{value}</label>)}</div><ErrorText>{errors.thirdPartyInvolved}</ErrorText></fieldset></div>}

function PolicyStep({draft,update,errors}:StepProps){
  const x=draft.policy;
  const set=(value:Partial<typeof x>)=>update("policy",{...x,...value});
  return <div className="p-5 sm:p-6">
    <div className="grid gap-x-5 gap-y-4 md:grid-cols-2">
      <Field label="Policy number" error={errors.policyNumber}><input className="field" value={x.policyNumber} onChange={e=>set({policyNumber:e.target.value})}/></Field>
      <Field label="Insurer" error={errors.insurer}><input className="field" value={x.insurer} onChange={e=>set({insurer:e.target.value})}/></Field>
      <div className="md:col-span-2"><Field label="Insurer wallet address" error={errors.insurerWallet}><input className="field font-mono" placeholder="0x..." value={x.insurerWallet??""} onChange={e=>set({insurerWallet:e.target.value})}/></Field></div>
      <Field label="Coverage type" error={errors.coverageType}><select className="field" value={x.coverageType} onChange={e=>set({coverageType:e.target.value})}><option>Comprehensive Motor</option><option>Motor Third Party</option><option>Fleet Motor</option></select></Field>
      <Field label="Insured asset / vehicle" error={errors.insuredAsset}><input className="field" value={x.insuredAsset} onChange={e=>set({insuredAsset:e.target.value})}/></Field>
      <Field label="Policy start date" error={errors.policyStartDate}><input className="field" type="date" value={x.policyStartDate} onChange={e=>set({policyStartDate:e.target.value})}/></Field>
      <Field label="Policy end date" error={errors.policyEndDate}><input className="field" type="date" value={x.policyEndDate} onChange={e=>set({policyEndDate:e.target.value})}/></Field>
      <MoneyField label="Coverage limit" value={x.coverageLimit} error={errors.coverageLimit} onChange={value=>set({coverageLimit:value})}/>
      <MoneyField label="Deductible" value={x.deductible} error={errors.deductible} onChange={value=>set({deductible:value})}/>
    </div>
    <div className="mt-5 rounded-xl border border-[#DCE6F1] bg-[#F5F8FC] px-4 py-3"><p className="eyebrow">Coverage summary</p><div className="mt-3 grid gap-3 sm:grid-cols-3"><Summary label="Coverage" value={x.coverageType}/><Summary label="Policy limit" value={naira(x.coverageLimit)}/><Summary label="Applicable deductible" value={naira(x.deductible)}/></div></div>
  </div>
}

function FinancialStep({draft,update,errors}:StepProps){const x=draft.financialLoss;const set=(value:Partial<typeof x>)=>update("financialLoss",{...x,...value});const total=documentedLoss(draft);return <div className="p-5 sm:p-6"><div className="grid gap-x-5 gap-y-4 md:grid-cols-2"><MoneyField label="Repair estimate" value={x.repairEstimate} error={errors.repairEstimate} onChange={value=>set({repairEstimate:value})}/><MoneyField label="Property damage" value={x.propertyDamage} error={errors.propertyDamage} onChange={value=>set({propertyDamage:value})}/><MoneyField label="Medical expenses" value={x.medicalExpenses} error={errors.medicalExpenses} onChange={value=>set({medicalExpenses:value})}/><MoneyField label="Other expenses" value={x.otherExpenses} error={errors.otherExpenses} onChange={value=>set({otherExpenses:value})}/></div><div className="mt-5 grid gap-4 rounded-[16px] border border-[#DCE6F1] bg-[#F5F8FC] p-4 md:grid-cols-2"><div><p className="eyebrow">Total documented loss</p><p className="mt-3 text-3xl font-semibold tracking-[-.045em] tabular-nums text-[#132033]">{naira(total)}</p><p className="mt-2 text-xs text-[#7B8899]">Calculated from the four entered loss categories.</p></div><MoneyField label="Requested claim amount" value={x.requestedAmount} error={errors.requestedAmount} onChange={value=>set({requestedAmount:value})}/></div></div>}

function MoneyField({label,value,error,onChange}:{label:string;value:number;error?:string;onChange:(value:number)=>void}){return <Field label={label} error={error}><div className="relative"><span className="pointer-events-none absolute inset-y-0 left-3.5 flex items-center text-sm font-semibold text-[#526177]">₦</span><input className="field !pl-9 tabular-nums" type="number" min="0" step="1000" value={value} onChange={e=>onChange(e.target.value===""?0:Number(e.target.value))}/></div></Field>}

function EvidenceStep({draft,update}:{draft:ClaimDraft;update:<K extends keyof ClaimDraft>(section:K,value:ClaimDraft[K])=>void}){const [file,setFile]=useState<File|null>(null);const [type,setType]=useState<EvidenceType>("Damage photographs");const [title,setTitle]=useState("");const [source,setSource]=useState("Claimant upload");const [description,setDescription]=useState("");const [formError,setFormError]=useState("");const [preview,setPreview]=useState<string|null>(null);const localFiles=useRef(new Map<string,File>());
  const addingRef=useRef(false);
  const [adding,setAdding]=useState(false);
  const add=async()=>{
    if(addingRef.current)return;
    if(!file||!title.trim()||!source.trim()){setFormError("Choose a file and provide a title and source.");return}
    addingRef.current=true;setAdding(true);setFormError("");
    try{
      await validateLocalEvidenceAddition(draft.evidence,file,async record=>{
        const selected=localFiles.current.get(record.id);
        if(selected)return selected;
        const stored=await evidenceBlobStorage.load(record.blobId??evidenceBlobId(record.filename,record.size,record.mimeType));
        return stored?.filename===record.filename?stored.blob:null;
      });
      const blobId=evidenceBlobId(file.name,file.size,file.type);
      await evidenceBlobStorage.save(blobId,file);
      const item:ClaimEvidenceRecord={id:crypto.randomUUID(),type,filename:file.name,title:title.trim(),description:description.trim(),source:source.trim(),createdAt:new Date().toISOString(),size:file.size,mimeType:file.type||"application/octet-stream",verificationStatus:"Uploaded",blobId};
      localFiles.current.set(item.id,file);
      update("evidence",[...draft.evidence,item]);setFile(null);setTitle("");setDescription("");
    }catch(caught){setFormError(caught instanceof Error?caught.message:"Unable to validate local evidence.")}
    finally{addingRef.current=false;setAdding(false)}
  };const replace=(id:string,value:Partial<ClaimEvidenceRecord>)=>update("evidence",draft.evidence.map(item=>item.id===id?{...item,...value}:item));return <div className="p-5 sm:p-6"><div className="grid gap-5 rounded-[16px] border border-dashed border-[#BFCDE0] bg-[#F8FAFD] p-4 md:grid-cols-2"><Field label="Evidence file"><input className="field file:mr-3 file:rounded-lg file:border-0 file:bg-[#EDF4FF] file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-[#326DCA]" type="file" accept={EVIDENCE_ACCEPT} disabled={adding} onChange={e=>{
    const selected=e.target.files?.[0]??null;setFormError("");
    try{if(selected)validateEvidenceFile(selected);setFile(selected)}
    catch(caught){setFile(null);e.target.value="";setFormError(caught instanceof Error?caught.message:"Invalid evidence file.")}
  }}/><p className="mt-2 text-xs text-[#7B8899]">{EVIDENCE_LIMITS_HELP} Draft files remain local and unverified.</p></Field><Field label="Evidence category"><select className="field" value={type} onChange={e=>setType(e.target.value as EvidenceType)}>{evidenceTypes.map(value=><option key={value}>{value}</option>)}</select></Field><Field label="Evidence title"><input className="field" placeholder="e.g. Front passenger-side damage" value={title} onChange={e=>setTitle(e.target.value)}/></Field><Field label="Source"><input className="field" value={source} onChange={e=>setSource(e.target.value)}/></Field><div className="md:col-span-2"><Field label="Description"><textarea className="field min-h-24 resize-y" value={description} onChange={e=>setDescription(e.target.value)}/></Field></div><div className="md:col-span-2 flex items-center justify-between"><ErrorText>{formError}</ErrorText><button type="button" onClick={()=>void add()} disabled={adding} className="btn-primary"><Paperclip size={15}/>Add evidence</button></div></div><div className="mt-5 space-y-3">{draft.evidence.length===0?<div className="rounded-[14px] border border-[#E1E7EF] p-6 text-center"><FileText className="mx-auto text-[#8EA0B5]" size={22}/><p className="mt-3 text-sm font-semibold">No evidence added yet</p><p className="mt-1 text-xs text-[#7B8899]">Add supporting records before review.</p></div>:draft.evidence.map(item=><article key={item.id} className="rounded-[14px] border border-[#E1E7EF] bg-white p-5"><div className="flex flex-wrap items-start gap-4"><span className="grid size-10 place-items-center rounded-xl bg-[#EDF4FF] text-[#326DCA]"><FileText size={17}/></span><div className="min-w-0 flex-1"><p className="font-semibold text-[#172437]">{item.title}</p><p className="mt-1 text-xs text-[#7B8899]">{item.filename} · {formatBytes(item.size)} · {new Date(item.createdAt).toLocaleString("en-NG")}</p><div className="mt-3 flex flex-wrap gap-3"><select aria-label={`Classification for ${item.title}`} className="rounded-lg border border-[#D6DFEA] bg-white px-3 py-2 text-xs" value={item.type} onChange={e=>replace(item.id,{type:e.target.value as EvidenceType})}>{evidenceTypes.map(value=><option key={value}>{value}</option>)}</select><span className="rounded-full bg-[#FFF5DE] px-3 py-2 text-[11px] font-semibold text-[#946513]">{item.verificationStatus}</span></div></div><button type="button" onClick={()=>setPreview(preview===item.id?null:item.id)} className="btn-secondary !min-h-9 !px-3">Metadata</button><button type="button" disabled={adding} onClick={()=>update("evidence",draft.evidence.filter(value=>value.id!==item.id))} className="grid size-9 place-items-center rounded-lg text-[#A14450] hover:bg-[#FBECEE]" aria-label={`Remove ${item.title}`}><Trash2 size={16}/></button></div>{preview===item.id&&<dl className="mt-4 grid gap-3 border-t border-[#E7ECF2] pt-4 text-xs sm:grid-cols-3"><Summary label="Source" value={item.source}/><Summary label="MIME type" value={item.mimeType}/><Summary label="Description" value={item.description||"No description supplied"}/></dl>}</article>)}</div></div>}

function ReviewStep({draft,update,walletAddress,onEdit,error}:{draft:ClaimDraft;update:<K extends keyof ClaimDraft>(section:K,value:ClaimDraft[K])=>void;walletAddress:string|null;onEdit:(step:ClaimStep)=>void;error?:string}){const total=documentedLoss(draft);return <div className="space-y-4 p-5 sm:p-6"><ReviewCard title="Incident summary" onEdit={()=>onEdit(0)} rows={[["Type",draft.incident.incidentType],["Date",draft.incident.incidentDate],["Location",draft.incident.location],["Vehicle",draft.incident.vehicleRegistration],["Third party",draft.incident.thirdPartyInvolved]]}/><ReviewCard title="Policy summary" onEdit={()=>onEdit(1)} rows={[["Policy",draft.policy.policyNumber],["Insurer",draft.policy.insurer],["Insurer wallet",draft.policy.insurerWallet||"Not provided"],["Coverage",draft.policy.coverageType],["Insured asset",draft.policy.insuredAsset],["Limit",naira(draft.policy.coverageLimit)]]}/><ReviewCard title="Financial-loss summary" onEdit={()=>onEdit(2)} rows={[["Documented loss",naira(total)],["Requested amount",naira(draft.financialLoss.requestedAmount)],["Currency","Nigerian naira (NGN)"]]}/><ReviewCard title="Evidence summary" onEdit={()=>onEdit(3)} rows={[["Records supplied",String(draft.evidence.length)],["Status",draft.evidence.length?"Uploaded · unverified":"No evidence supplied"]]}/><div className="grid gap-4 rounded-[16px] border border-[#DCE6F1] bg-[#F5F8FC] p-5 sm:grid-cols-2"><Summary label="Connected wallet" value={walletAddress?shortenAddress(walletAddress):"Not connected"}/><Summary label="Current GenLayer network" value={BRADBURY_LABEL}/></div><label className={`flex cursor-pointer items-start gap-3 rounded-[16px] border p-5 ${draft.review.declarationAccepted?"border-[#8ECDB8] bg-[#F0FAF6]":"border-[#D6DFEA] bg-white"}`}><input type="checkbox" className="mt-0.5 size-4 accent-[#4D8DFF]" checked={draft.review.declarationAccepted} onChange={e=>update("review",{...draft.review,declarationAccepted:e.target.checked})}/><span className="text-sm leading-6 text-[#435168]">I confirm that the information and evidence supplied in this claim are accurate to the best of my knowledge.</span></label><ErrorText>{error}</ErrorText><div className="rounded-xl bg-[#FFF8E8] p-4 text-xs leading-5 text-[#806225]">Submission requests your wallet signature and writes the claim to the live ClaimGuard intelligent contract on GenLayer Bradbury. Browser-only evidence is not committed until it has a validator-accessible HTTPS manifest.</div></div>}
...
function Summary({label,value}:{label:string;value:string}){const wallet=useWallet();const displayValue=label==="Current GenLayer network"?(wallet.isOnBradbury?BRADBURY_LABEL:wallet.chainId?`Wrong network (${wallet.chainId})`:"Not connected"):value;return <div><dt className="text-[10px] uppercase tracking-[.08em] text-[#65758A]">{label}</dt><dd className="mt-1 text-sm font-semibold text-[#26364D]">{displayValue||"—"}</dd></div>}
function FollowingStages({current}:{current:ClaimStep}){
  const items=[{n:"02",t:"Policy",d:"Coverage and dates"},{n:"03",t:"Financial Loss",d:"Loss and requested amount"},{n:"04",t:"Evidence",d:"Supporting records"},{n:"05",t:"Review",d:"Check and submit"}].slice(current);
  return <section aria-label="Following stages" className="rounded-xl border border-[#DDE4ED] bg-[#F0F4F9] px-4 py-3">
    <p className="eyebrow !text-[10px]">Up next</p>
    <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-3 sm:flex sm:flex-wrap sm:gap-5">{items.map(item=><div key={item.n} className="flex min-w-0 items-start gap-2 sm:flex-1"><span className="mt-0.5 font-mono text-[10px] font-semibold text-[#3975CC]">{item.n}</span><div><p className="text-xs font-semibold text-[#344359]">{item.t}</p><p className="mt-0.5 text-[11px] leading-4 text-[#62748B]">{item.d}</p></div></div>)}</div>
  </section>
}
